...
export class Farm extends GameObjectClass {
  constructor(properties) {
    ...
    this.totalUpdates = 0;
    this.circumference =
      this.width * gridCellSize * 2 + this.height * gridCellSize * 2;
    this.numIssues = 0;
    this.assignedPeople = [];
    this.points = [];
  ...
  assignWarn() {
    ...
    if (this.hasWarn) {
      ...
      if (warnedAnimals.length && this.numIssues < warnedAnimals.length) {
        warnedAnimals[
          Math.floor(Math.random() * warnedAnimals.length)
        ].hideWarn();
      }

      if (
        notWarnedAnimals.length &&
        this.numIssues > adults.length - notWarnedAnimals.length
      ) {
        notWarnedAnimals[
          Math.floor(Math.random() * notWarnedAnimals.length)
        ].showWarn();
      }
    }
  }
  ...
  update(gameStarted, updateCount) {
    ...
    if (gameStarted) {
      ...
      this.demand +=
        this.children.length - 1 + (updateCount * updateCount) / 1e9;
    ...
    for (let i = 0; i < this.numIssues; i++) {
      ...
      const atHomePeopleOfSameType = people.filter(
        (person) => person.atHome && person.type === this.type,
      );
      ...
      for (let j = 0; j < atHomePeopleOfSameType.length; j++) {
        ...
        if (thisRoute && thisRoute.length === bestRoute.length) {
          ...
          if (atHomePeopleOfSameType[j].parent !== closestPerson.parent) {
            const bestDistance = bestRoute.reduce(
              (acc, curr) => acc + (curr.distance ?? 0),
              0,
            );
            const thisDistance = thisRoute.reduce(
              (acc, curr) => acc + (curr.distance ?? 0),
              0,
            );
  ...
  addToSvg() {
    const x =
      this.x * gridCellSize + fenceLineThickness / 2 + gridLineThickness / 2;
    const y =
      this.y * gridCellSize + fenceLineThickness / 2 + gridLineThickness / 2;
    const svgWidth =
      gridCellSize * this.width - fenceLineThickness - gridLineThickness;
    const svgHeight =
      gridCellSize * this.height - fenceLineThickness - gridLineThickness;
    ...
    this.pinSvg.translate = `${x + svgWidth / 2}px, ${
      y + svgHeight / 2 + 1.5
    }px`;
  ...
  updateWarn() {
    ...
    const dashoffset =
      fullCircle - (fullCircle / maxOverflow) * numOverflowIssues;
